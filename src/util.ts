...
export const excludeFilterDataFrameRows = (
  ...
  if (searchableColIndex === -1) {
    // This table doesn't have a column for specified filterType
    return [...data];
  }
  const filteredRows = data.slice(1).filter(row => {
    ...
    return (
      cellText && !cellText.toLowerCase().includes(filterVal.toLowerCase())
    );
  });
  return [header].concat(filteredRows);
};

export const filterDataFrameRows = (
  filterType: string,
  filterVal: string,
  data: DataFrame
) => {
